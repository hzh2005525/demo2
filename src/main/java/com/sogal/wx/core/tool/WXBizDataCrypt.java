...
public class WXBizDataCrypt {
     ...
     * @throws Exception
     */
    public JSONObject decrypt(String encryptedData, String iv) throws BizException {
        String jsonStr;
        ...
        try {
            ...
            jsonStr = new String(original, "GBK");

            jsonObject = new JSONObject(jsonStr);
            if (!jsonObject.getJSONObject("watermark").get("appid").toString().equals(this.appId)){
                throw new BizException(WechatCoreError.CRYPTO_ERROR);
            }
        } catch (Exception ex) {
            log.error("解密异常", ex);
            throw new BizException(WechatCoreError.CRYPTO_ERROR);
