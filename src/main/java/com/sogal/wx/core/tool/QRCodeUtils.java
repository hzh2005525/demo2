...
public class QRCodeUtils {
    ...
    public static File createQrcodeMy(String content) {
    ...
    public static File createQrcodeWithLogoByURI(String content,String uriStr) {

        try {
            File file = createQrcodeMy(content);
            BufferedImage image = ImageIO.read(file);
            BufferedImage logo = null;
            try {
                if(StringUtils.isNotBlank(uriStr)&& uriStr.startsWith("http")){
                    logger.info("头像uri={}",uriStr);
                    URL avatarUrl = new URL(uriStr);
                    logo = ImageIO.read(avatarUrl);
                }else {
                    logger.info("头像取默认头像");
                    ClassPathResource redResourceOfAvatar = new ClassPathResource("avatar.png");
                    logo = ImageIO.read(redResourceOfAvatar.getInputStream());
                }
                overlapImageWithLogoBufferImage(image,FORMAT, file.getAbsolutePath(), logo,
                        new MatrixToLogoImageConfig(Color.WHITE,5));
            } catch (Exception e) {
                logger.warn("二维码添加logo时发生异常:logo资源获取失败");
                return file;
//                throw new IOException("二维码添加logo时发生异常！", e);
            }
            return file;
    ...
    private static void overlapImageWithLogoBufferImage(BufferedImage image, String format, String absolutePath, BufferedImage logo, MatrixToLogoImageConfig logoConfig) throws IOException {
        Graphics2D g = image.createGraphics();
        // 考虑到logo图片贴到二维码中，建议大小不要超过二维码的1/5;
        int width = image.getWidth() / logoConfig.getLogoPart();
        int height = image.getHeight() / logoConfig.getLogoPart();
        ...
        g.dispose();
        // 写入logo图片到二维码
        ImageIO.write(image, format, new File(absolutePath));
    }
}
