 ...
 */
@Slf4j
public class WkcBaseQRCode implements QRCodeDrawingBoard {

    @Resource
    private WxMaService wxWkcMaService;

    @Value("${image.server.wkc.share-path}")
...
//            File file = wxMaService.getQrcodeService().createWxaCodeUnlimit(scene, qrCodeBody.getShareUrl());
            File file = wxWkcMaService.getQrcodeService().createWxaCodeUnlimit(scene, qrCodeBody.getShareUrl(),430,true,(WxMaCodeLineColor)null,true);
