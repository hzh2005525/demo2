 ...
 */
@Slf4j
public class QwcBaseQRCode implements QRCodeDrawingBoard {

    @Resource
    private WxMaService wxQwcMaService;

    @Value("${image.server.qwc.share-path}")
...
//            File file = wxMaService.getQrcodeService().createWxaCodeUnlimit(scene, qrCodeBody.getShareUrl());
            File file = wxQwcMaService.getQrcodeService().createWxaCodeUnlimit(scene, qrCodeBody.getShareUrl(),430,true,(WxMaCodeLineColor)null,true);
