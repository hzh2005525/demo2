 ...
 */
@Slf4j
public class MjcBaseQRCode implements QRCodeDrawingBoard {

    @Resource
    private WxMaService wxMjcMaService;

    @Value("${image.server.mjc.share-path}")
...
//            File file = wxMaService.getQrcodeService().createWxaCodeUnlimit(scene, qrCodeBody.getShareUrl());
            File file = wxMjcMaService.getQrcodeService().createWxaCodeUnlimit(scene, qrCodeBody.getShareUrl(),430,true,(WxMaCodeLineColor)null,true);
